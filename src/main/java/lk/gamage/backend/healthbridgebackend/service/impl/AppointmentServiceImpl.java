...
public class AppointmentServiceImpl implements AppointmentService {
    ...
    public List<Appointment> find(String patientId, String doctorId, String status) {
        return repository.findAll().stream().filter(a -> patientId == null || patientId.isBlank() || patientId.equals(a.getPatientId()))
            ...
            .peek(this::normalizeDecision)
            .sorted(Comparator.comparing(Appointment::getAppointmentDate).thenComparing(Appointment::getAppointmentTime)).toList();
    }
    public Appointment findById(String id) { return repository.findById(id).map(this::normalizeDecision).orElseThrow(() -> new ResourceNotFoundException("Appointment not found: " + id)); }
    ...
    public Appointment decide(String id, String decision) {
        Appointment a = findById(id);
        DoctorDecision next;
        try { next = DoctorDecision.valueOf(decision.trim().toUpperCase()); }
        catch (Exception e) { throw new BadRequestException("Decision must be ACCEPTED or REJECTED."); }
        if (next == DoctorDecision.PENDING) throw new BadRequestException("Decision must be ACCEPTED or REJECTED.");
        if (a.getStatus() == AppointmentStatus.COMPLETED) throw new BadRequestException("Completed appointments cannot be changed.");
        a.setDoctorDecision(next);
        a.setStatus(next == DoctorDecision.REJECTED ? AppointmentStatus.CANCELLED : AppointmentStatus.UPCOMING);
        if (next == DoctorDecision.REJECTED) a.setCancellationReason("Rejected by doctor.");
    ...
    public Appointment complete(String id) {
        Appointment a = findById(id);
        if (a.getStatus() == AppointmentStatus.CANCELLED) throw new BadRequestException("Cancelled appointments cannot be completed.");
        a.setDoctorDecision(DoctorDecision.ACCEPTED);
        a.setStatus(AppointmentStatus.COMPLETED);
        a.setUpdatedAt(LocalDateTime.now());
        return repository.save(a);
    }
    private Appointment normalizeDecision(Appointment appointment) {
        if (appointment.getDoctorDecision() == null) {
            appointment.setDoctorDecision(appointment.getStatus() == AppointmentStatus.COMPLETED
                    ? DoctorDecision.ACCEPTED
                    : appointment.getStatus() == AppointmentStatus.CANCELLED ? DoctorDecision.REJECTED : DoctorDecision.PENDING);
        }
        return appointment;
    }
